...
public class LoginLib
{
    ...
    public static LoginArguments GetLoginArguments(string[] args)
    {
        ...
        { 
            AccountId = ParameterLib.GetParameter(args, "-accid", string.Empty),
            Base64Login = ParameterLib.GetParameter(args, "-b64", string.Empty),
            Email = ParameterLib.GetParameter(args, "-username", string.Empty) ?? ParameterLib.GetParameter(args, "-user", string.Empty) ?? ParameterLib.GetParameter(args, "-email", string.Empty),
            Password = ParameterLib.GetParameter(args, "-password", string.Empty) ?? ParameterLib.GetParameter(args, "-pass", string.Empty),
            rememberDevice = ParameterLib.HasParameter(args, "-remember-device"),
            rememberMe = ParameterLib.HasParameter(args, "-remember-me"),
            trustedDeviceId = ParameterLib.GetParameter(args, "-trustedid", string.Empty),
            trustedDeviceName = ParameterLib.GetParameter(args, "-trustedname", string.Empty),
            useFileStorage = ParameterLib.HasParameter(args, "-usefilestore"),
            printStorage = ParameterLib.HasParameter(args, "-printstore"),
            rmTicket = ParameterLib.GetParameter(args, "-rmticket", string.Empty),
        };
    }
